...
const DateView = props => {
  const { defaultDateView, date, format, onDateChange, maxDate, minDate, primaryColor, secondaryColor, primaryTextColor, secondaryTextColor } = props;
          ...
          <Body key={k}>{week.map((day, i) => {
            return (
              (currentDate === day.d) && !d.isMax ? <SelectedSpan
                ...
                isMin={day.isMin}
                key={i}
                onClick={() => day && !d.isMax && !day.isMin && onDateChange(day.d)}
                primaryColor={primaryColor}
                primaryTextColor={primaryTextColor}
